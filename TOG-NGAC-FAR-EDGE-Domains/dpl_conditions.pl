...
:- module(dpl_conditions,[c_assign/3,c_associate/4, built_in_relation_name/1,
...
evaluate_condition(C) :- (C==true; C=='True'), !.
evaluate_condition(C) :- (C==false; C=='False'), !, fail.
evaluate_condition(C) :-
    validate_built_in_relation(C), !,
...
%
validate_built_in_relation( R ) :- R =.. [RN,X,Y],
    built_in_relation_name(RN),
    (   validate_constant(X) ; validate_condition_variable(X)  ),
    (   validate_constant(Y) ; validate_condition_variable(Y)  ),
    !.

built_in_relation_name(N) :-
    built_in_relations(Rs), R=..[N,_,_],
    memberchk(R,Rs).

built_in_relations([is_equal_to(any,any),
...
boolean_constant(X) :- (X==true;X=='True';X==false;X=='False').

validate_condition_variable(V) :- atom(V),
    defined_condition_variables(CVs), memberchk(V:_,CVs).

validate_condition_predicate(P,Nargs) :-
    P =.. [PN|Pargs], length(Pargs,Nargs),
    compound_name_arity(GP,PN,Nargs),
    GP =.. [PN|Gargs],
    % instead of following 2, could do: condition_predicate(PN,Gargs),
...
install_condition_predicates :- % assumes predefined predicates already installed
    defined_condition_predicates(CurrentPredicates),
    findall(Predicate,
            (   condition_predicate(PredName, PredArgs), Predicate=..[PredName|PredArgs] ),
            Predicates),
...
install_condition_variables :-
    findall(CondVar, condition_variable(CondVar), CondVars),
    %findall(CondVar, (condition_variables(Vars), member(CondVar,Vars)), CondVars),
