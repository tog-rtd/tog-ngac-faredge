...
# #### register a new log
#send a log with message manually encoded:	
 curl -H "Authorization: OAuth ${mytoken}" -H "Content-Type: text/plain"   -XPOST http://${server}:${execmanager_port}/new_log?code=111\&ip="10.11.12.13"\&message="Aa%20aa"\&user="jaja@abc.com";

 
#we encode the message with a function
	mymessage=`rawurlencode "bbb bbb"`;
	echo "mymessage is ${mymessage}";

# #### register a new log
curl -H "Authorization: OAuth ${mytoken}" -H "Content-Type: text/plain"  -XPOST http://${server}:${execmanager_port}/new_log?code=111\&ip="10.11.12.13"\&message=${mymessage}\&user="jaja@abc.com";
