...
syntax(import_erp(erp_file),			              advanced).
...
semantics(import_erp(ERP)) :- atom(ERP).
...
help(import_erp, '"import" an event-response package from a file').
...
do(getpol) :- !, param:current_policy(P), writeln(P).

do(help) :- help.
do(help) :- !.
do(help(C)) :- !, show_help(C).

do(import(erp(ERLfile))) :- do(import_erp(ERLfile)).
do(import(pm(PM))) :- do(import_pm(PM)).
do(import(policy(P))) :- do(import_policy(P)).

do(import_erp(ERLfile)) :- % import an event-response package
