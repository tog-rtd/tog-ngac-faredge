...
:- module(epp_era, [activate_loaded_erp/1, deactivate_loaded_erp/1,
                    activate_erp/2,deactivate_erp/1,report_event/1]).
...
% ------------------------------------------------------------------------
% epp_era initialization - called by module ngac
%
...
deactivate_loaded_erp(_ERPname) :- !,
    true.
...
event(event(Uspec,PCspec,OPspec,Ospec)) :- % check whether valid event
    uspec(Uspec), pcspec(PCspec), opspec(OPspec), ospec(Ospec).
...
event_match( event(E_U, E_PC, E_Op, E_O), ev_pat(P_U, P_PC, P_Op, P_O), PC ) :- !,
    match_pc(E_PC, P_PC),
...
do_admin_command(log(M),_,_,_) :- !, format('epp log: ~q~n',M).
do_admin_command(_,_,_,_).
